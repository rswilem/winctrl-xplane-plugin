#include "c172-laminar-fcu-efis-profile.h"
...
C172LaminarFCUEfisProfile::C172LaminarFCUEfisProfile(ProductFCUEfis *product) : FCUEfisAircraftProfile(product) {
...
bool C172LaminarFCUEfisProfile::IsEligible() {
    return Dataref::getInstance()->exists("laminar/c172/electrical/battery_amps");
}

const std::vector<std::string> &C172LaminarFCUEfisProfile::displayDatarefs() const {
...
const std::unordered_map<uint16_t, FCUEfisButtonDef> &C172LaminarFCUEfisProfile::buttonDefs() const {
...
void C172LaminarFCUEfisProfile::updateDisplayData(FCUDisplayData &data) {
...
void C172LaminarFCUEfisProfile::buttonPressed(const FCUEfisButtonDef *button, XPLMCommandPhase phase) {
